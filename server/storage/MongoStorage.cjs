...
class MongoStorage {
    async getParcel(id) {
        const collection = await (0, mongodb_1.getCollection)('parcels');
    ...
    async updateParcel(id, updates) {
        const collection = await (0, mongodb_1.getCollection)('parcels');
        const result = await collection.findOneAndUpdate({ id }, { $set: { ...updates, updatedAt: new Date() } }, { returnDocument: 'after' });
    ...
    async getBusinessRules() {
        const collection = await (0, mongodb_1.getCollection)('businessRules');
    ...
    async updateBusinessRules(id, insertRules) {
        const collection = await (0, mongodb_1.getCollection)('businessRules');
        const result = await collection.findOneAndUpdate({ id }, { $set: { ...insertRules, updatedAt: new Date() } }, { returnDocument: 'after' });
        return result || undefined;
    }
    async getDepartment(id) {
        const collection = await (0, mongodb_1.getCollection)('departments');
    ...
    async resetToDefaults() {
        const [parcelsCollection, businessRulesCollection, departmentsCollection] = await Promise.all([
            (0, mongodb_1.getCollection)('parcels'),
            (0, mongodb_1.getCollection)('businessRules'),
            (0, mongodb_1.getCollection)('departments')
        ]);
