...
class MemStorage {
    ...
    async getAllParcels() {
        return Array.from(this.parcels.values()).sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
    }
    async getParcelsByDepartment(department) {
        return Array.from(this.parcels.values()).filter(parcel => parcel.department === department);
    }
    async getParcelsByStatus(status) {
        return Array.from(this.parcels.values()).filter(parcel => parcel.status === status);
    }
...
exports.storage = new MongoStorage_1.MongoStorage();
